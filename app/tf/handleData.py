...
with open(sys.argv[1]) as rawDataFile:
...
newData = np.zeros((rowCount, w1+w3+w4+w6+w7+w9+w10+w12+w14+w15+w17+w21+9))

# populate the matrix with the new columns
c = 0; # index of current column (relative to old data)
newData[:,c:c+w1] = col1.reshape((rowCount, w1)); c=c+w1                 # 1
newData[:, c] = col2; c = c + 1                                      # 2
newData[:, c:c+w3]=col3.reshape((rowCount, w3)); c=c+w3                  # 3
newData[:, c:c+w4]=col4.reshape((rowCount, w4)); c=c+w4                  # 4
newData[:, c] = col5; c=c + 1                                        # 5
newData[:, c:c+w6]=col6.reshape((rowCount, w6)); c=c+w6                 # 6                                                               # 6
newData[:, c:c+w7]=col7.reshape((rowCount, w7)); c=c+w7                  # 7
newData[:, c] = col8; c = c + 1                                      # 8
newData[:, c:c+w9]=col9.reshape((rowCount, w9)); c=c+w9                  # 9
newData[:, c:c+w10]=col10.reshape((rowCount, w10)); c=c+w10                 # 10
newData[:, c] = col11; c = c + 1                                      # 11
newData[:, c:c+w12]=col12.reshape((rowCount, w12)); c=c+w12             # 12
newData[:, c] = col13; c = c + 1                                      # 13
newData[:, c:c+w14]=col14.reshape((rowCount, w14)); c=c+w14             # 14
newData[:, c:c+w15]=col15.reshape((rowCount, w15)); c=c+w15             # 15
newData[:, c] = col16; c = c + 1                                      # 16
newData[:, c:c+w17]=col17.reshape((rowCount, w17)); c=c+w17                 # 17
newData[:, c] = col18; c = c + 1                                        # 18
newData[:, c] = col19; c = c + 1                                        # 19
newData[:, c] = col20; c = c + 1                                        # 20
newData[:, c:c+w21] = col21.reshape(rowCount, w21)                        # 21

# Save to csv file
np.savetxt(sys.argv[2], newData, delimiter=",")
